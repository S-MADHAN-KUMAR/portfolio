...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      ...
      >
        <div className="relative w-full h-full rounded-full overflow-hidden shadow-lg border-2 border-white/20 bg-neutral-900 flex items-center justify-center p-2">
          <img
            src={image.src}
            alt={image.alt}
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl max-w-sm w-full overflow-hidden shadow-2xl relative"
        ...
        >
          <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-4 right-4 w-8 h-8 bg-neutral-800 rounded-full text-white flex items-center justify-center hover:bg-neutral-700 transition-all cursor-pointer z-10"
            >
              <X size={16} />
            </button>
          <div className="relative aspect-square mb-6 bg-neutral-800 rounded-xl p-8 flex items-center justify-center">
            <img
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="text-center">
              {selectedImage.title && (
                <h3 className="text-2xl font-black uppercase tracking-tighter text-white mb-2">{selectedImage.title}</h3>
              )}
              {selectedImage.description && (
                <p className="text-neutral-400 text-sm font-medium uppercase tracking-wide">{selectedImage.description}</p>
              )}
            </div>
          )}
        </div>
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className={`relative select-none cursor-grab active:cursor-grabbing flex items-center justify-center ${className}`}
        style={{
          width: '100%',
          height: '100%',
